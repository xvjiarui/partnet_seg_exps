...
parser.add_argument('--model', type=str, default="model", help='model to use [default: model_ins]')
...
print('Category: {}'.format(' and '.join(FLAGS.category)))
train_h5_fn_list = []
val_h5_fn_list = []
for category in FLAGS.category:
    ...
    for item in os.listdir(data_in_dir):
        if item.endswith('.h5') and item.startswith('train-'):
            train_h5_fn_list.append(os.path.join(data_in_dir, item))
    for item in os.listdir(data_in_dir):
...
def load_data(fn):
    if fn in all_data_cache.keys():
        ...
        semseg_one_hot = np.ones((semseg_one_hot.shape[0], semseg_one_hot.shape[1], 1))
    ...
    with tf.Graph().as_default():
        ...
        def train_one_epoch(epoch_num):
            ...
            for item in train_h5_fn_list:
                printout(flog, 'Reading data from: %s' % item)

                pts, semseg_one_hot, semseg_mask, insseg_one_hot, insseg_mask = load_data(item)
